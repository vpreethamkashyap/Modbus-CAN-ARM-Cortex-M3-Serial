...
void Modbus_CAN_IntHandler(void)
{
    ...
    if(can_status == CAN_INT_INT0ID_STATUS)
    {
...
void Modbus_CAN_Init(enum Modbus_CAN_BitRate bit_rate, unsigned char slave_number)
{
		...
		slave = slave_number;      
                ...
                CANInit(MODBUS_CAN);
                //Set bit timing
                CANBitTimingSet(MODBUS_CAN, &modbus_canbit);
...
void Modbus_CAN_SetBitRate(enum Modbus_CAN_BitRate bit_rate)
{
      ...
      {
            case MODBUS_100KBPS:
                          modbus_canbit.uSyncPropPhase1Seg = 5; //3 *tq; following the example from the microcontroller's pdf
						  //.ulSyncPropPhase1Seg = 5;
                          modbus_canbit.uPhase2Seg = 4;
						  //.ulPhase2Seg = 4;
                          modbus_canbit.uSJW = 4;
						  //.ulSJW = 4;
                          modbus_canbit.uQuantumPrescaler = 8;
						  //.ulQuantumPrescaler = 8;
            ...
            case MODBUS_1MBPS:
                          modbus_canbit.uSyncPropPhase1Seg = 5;      //3 *tq; following the example from the microcontroller's pdf
                          modbus_canbit.uPhase2Seg = 2;
                          modbus_canbit.uSJW = 2;
                          modbus_canbit.uQuantumPrescaler = 1;
